---
title: "Expanding Transportation and Socioeconomic Patterns"
editor_options: 
...
#| echo: false
#| output: false
if (!exists("ValidTablesYears")) {
  ValidTablesYears <- pin_read(BoardLocal, "ValidTablesYears")
}
```
...
#| echo: false
#| output: false
if (!exists("ValidTablesLastYear")) {
  ValidTablesLastYear <- pin_read(BoardLocal, "ValidTablesLastYear")
}
```
...
ggplot(VariablesPerDataset) +
  ...
  scale_fill_manual(
    values = c("2022" = Params$ColorHighlight, "2020" = Params$ColorGray)
  ) +
  scale_x_continuous(
...
It's clear that **there is one way we can read the variables one by one** and hypothesize the variables to select. Instead, we are going to follow some steps to reduce the number of variable and the classify them in to groups to select.
...
#| output: asis
ConceptsToExplore[
  nchar(concept) == max(nchar(concept)),
  paste0("> ", concept)
] |>
  cat()
```
...
#| echo: false
ConceptsToExploreValidConceptLength[
  concept_length == max(concept_length),
  paste0("- `", concept, "`")
] |>
  head() |>
  cat(sep = "\n")
...
#| echo: false
#| output: false
if (!exists("ConceptsToExploreByDistance")) {
  ConceptsToExploreByDistance <- pin_read(
    BoardLocal,
    "ConceptsToExploreByDistance"
  )
}
...
To understand what kind of variables we have in each cluster lets use the **statistic tf-idf** to list the **top 5 words** (after removing stop words) of each cluster and use that reference to select the most meaningful ones for this context.
...
#| echo: false
#| output: false
if (!exists("AcsVariableTracts")) {
  AcsVariableTracts <- pin_read(BoardLocal, "AcsVariableTracts")
}

if (!exists("DecVariableTracts")) {
  DecVariableTracts <- pin_read(BoardLocal, "DecVariableTracts")
}
```
...
#| echo: false
#| output: false
if (!exists("ConsolidatedVariableTractsJoined")) {
  ConsolidatedVariableTractsJoined <- pin_read(
    BoardLocal,
    "ConsolidatedVariableTractsJoined"
  )
}
...
```{r}
#| echo: false
if (!exists("ConsolidatedVariableTractsByZoneId")) {
  ConsolidatedVariableTractsByZoneId <- pin_read(
    BoardLocal,
    "ConsolidatedVariableTractsByZoneId"
  )
}
...
#| echo: false
#| output: false
if (!exists("PuVariableTractsMeanDiff")) {
  PuVariableTractsMeanDiff <- pin_read(BoardLocal, "PuVariableTractsMeanDiff")
}

if (!exists("DoVariableTractsMeanDiff")) {
  DoVariableTractsMeanDiff <- pin_read(BoardLocal, "DoVariableTractsMeanDiff")
}
```
...
BoardLocal |>
  pin_write(
    ConsolidatedVariableTractsByZoneId[,
      .SD,
      .SDcols = c("LocationID", TopVariableTractsMeanDiff)
    ],
