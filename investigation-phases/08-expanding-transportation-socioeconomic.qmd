...
Asc1BasicConcept <-
  ValidTablesLastYear[dataset == "acs1"
                      & year == 2022
                      & !concept %like% paste(IgnoreRaceVariables, collapse = "|")
                      & !label %like% "Male|Female",
...
To facilitate the manual exploration let's divide the **`r nrow(Asc1BasicConcept)``** variables in to 20 clusters of variables with similar words.

```{r}
ClusterCheck <-
  kmeans(Asc1BasicConceptWordDistance,
         centers = 20, 
         iter.max = 200)

Asc1BasicConcept[, cluster := ClusterCheck$cluster]
setkey(Asc1BasicConcept, cluster)
```
