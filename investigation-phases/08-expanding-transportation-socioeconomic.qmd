...
```{r}
ZonesShapes <-
  pin_read(BoardRemote, "ZonesShapes") |>
  (\(df) {
    df[df[["borough"]] %chin% BoroughSelected, c("OBJECTID", "LocationID")]
  })() |>
...
if (!exists("ValidTablesYears")) {
  ValidTablesYears <- pin_read(BoardRemote, "ValidTablesYears")
}
```
...
if (!exists("ValidTablesLastYear")) {
  ValidTablesLastYear <- pin_read(BoardRemote, "ValidTablesLastYear")
}
```
...
ggplot(VariablesPerDataset) +
  ...
  scale_fill_manual(values = c("2022" = ColorHighlight, "2020" = ColorGray)) +
  scale_x_continuous(
...
if (!exists("ConceptsToExploreByDistance")) {
  ConceptsToExploreByDistance <- pin_read(BoardRemote, "ConceptsToExploreByDistance")
}
```
...
if (!exists("AcsVariableTracts")) {
  AcsVariableTracts <- pin_read(BoardRemote, "AcsVariableTracts")
}

if (!exists("DecVariableTracts")) {
  DecVariableTracts <- pin_read(BoardRemote, "DecVariableTracts")
}
```
...
if (!exists("ConsolidatedVariableTractsJoined")) {
  ConsolidatedVariableTractsJoined <- pin_read(BoardRemote, "ConsolidatedVariableTractsJoined")
}
```
...
if (!exists("ConsolidatedVariableTractsByZoneId")) {
  ConsolidatedVariableTractsByZoneId <- pin_read(BoardLocal, "ConsolidatedVariableTractsByZoneId")
}
...
PuVariableTractsMeanDiff <-
  ConsolidatedVariableTractsByZoneId[
    TrainingSample[, .(LocationID = PULocationID, take_current_trip)],
    on = "LocationID"
  ][,
    lapply(.SD, \(x) {
      if (uniqueN(x) == 1L) {
...
DoVariableTractsMeanDiff <-
  ConsolidatedVariableTractsByZoneId[
    TrainingSample[, .(LocationID = DOLocationID, take_current_trip)],
    on = "LocationID"
  ][,
    lapply(.SD, \(x) {
      if (uniqueN(x) == 1L) {
...
if (!exists("PuVariableTractsMeanDiff")) {
  PuVariableTractsMeanDiff <- pin_read(BoardRemote, "PuVariableTractsMeanDiff")
}

if (!exists("DoVariableTractsMeanDiff")) {
  DoVariableTractsMeanDiff <- pin_read(BoardRemote, "DoVariableTractsMeanDiff")
}
```
