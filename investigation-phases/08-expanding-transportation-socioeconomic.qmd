...
Asc1BasicConcept[concept_length == 70,
                 concept]
```
...
```{r}
ShortVariableNames <- Asc1BasicConcept[concept_length <= 70, concept]

Asc1BasicConceptHash <- 
  tolower(ShortVariableNames) |>
  strsplit("\\s+") |> 
  hashr::hash()
...
```{r}
rownames(Asc1BasicConceptWordDistance) <- ShortVariableNames
colnames(Asc1BasicConceptWordDistance) <- ShortVariableNames
...
NumberOfVariablesByDistance[, `Proportion of Variables` := `Number of Variables`/ length(ShortVariableNames)]
...
```{r}
VariablesToKeep <- Asc1WordDistanceDt[`Jaccard Distance` <= 0.35]
...
Asc1Graph <- graph_from_data_frame(VariablesToKeep, directed = FALSE)
Asc1GraphCluster <- cluster_edge_betweenness(Asc1Graph)
V(Asc1Graph)$color <- Asc1GraphCluster$membership

Asc1GraphData <- toVisNetworkData(Asc1Graph)
Asc1GraphData$nodes$title <-Asc1GraphData$nodes$label

visNetwork(nodes = Asc1GraphData$nodes,
           edges = Asc1GraphData$edges) |>
