...
# Create the contest-winning gt table directly from your data.table
  VariablesToUse[.(sub("_estimate(_(low|high))*", "", TopVariableTractsMeanDiff) |> unique()), 
    ...
    # Add row groups for better organization
  ][, category := fcase(
    grepl("Earnings|Income", concept), "💰 Economic Indicators",
    grepl("Industry", concept), "🏭 Industry Demographics", 
    grepl("transportation", concept, ignore.case = TRUE), "🚗 Transportation Patterns",
    default = "📊 Other Demographics"
  )] |>
  
  gt(groupname_col = "category") |>
  
  # Add a stunning header
  tab_header(
    title = md("**🏆 Top Census Variables**"),
    subtitle = md("*Key demographic and economic variables across clusters*")
  ) |>
  ...
  # Hide the category column since it's used for grouping
  cols_hide(category) |>
  
  # Style the table
  tab_style(
    style = cell_fill(color = ColorHighlight),
  ...
  # Style the variable names with monospace font
  tab_style(
    style = cell_text(font = "monospace", size = px(11)),
    locations = cells_body(columns = name)
  ) |>
  
  # Wrap long text in label column
  tab_style(
  ...
  # Add table options for better presentation
  tab_options(
  ...
  # Add a source note
  tab_source_note(
    source_note = md("**Source:** U.S. Census Bureau, American Community Survey (ACS) 5-Year Estimates")
  )
```
