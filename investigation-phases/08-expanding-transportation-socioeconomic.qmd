...
VariablesToUse[, .(N = comma(.N)), by = "survey"]
```

### Download data for Manhattan, Queens and Brooklyn with geometry

Now before creating function to get the data for thousends of variables, let's validate the workflow with one variable. In this case let's use the `B08013_001` to explore travel time to work.

```{r}
#| echo: false
#| output: false

TravelTimeToWorkTractFilePath <- here("output/cache-data/08-expanding-transportation-socioeconomic/TravelTimeToWorkTract.qs")

if(file.exists(ValidTablesLastYearFilePath)) {
  TravelTimeToWorkTract <- qs2::qs_read(TravelTimeToWorkTractFilePath)
}
```
...
TravelTimeToWorkTract <-
  get_acs(geography = "tract", 
          variables = c("B08013_001","B08006_001"),
          year = 2020, 
          state = "NY",
...
TravelTimeToWorkTract <- TravelTimeToWorkTract[!is.na(TravelTimeToWorkTract[["estimate"]]), ]
```
...
qs2::qs_save(TravelTimeToWorkTract, TravelTimeToWorkTractFilePath)
```

By plotting the results we can confirm that:

- We have values for the majority of our studied area
- We have different values from tract to tract
- The track tracks have different sizes
...
tm_shape(TravelTimeToWorkTract) +
  tm_polygons(fill = "estimate")
```

As result we need to join to the original zones of our training set in to 2 steps:
...
TravelTimeToWorkTractJoinedList <-
  lapply(list(within = st_within,
              overlaps = st_overlaps), 
         FUN = st_join,
         x = TravelTimeToWorkTract,
         y = ZonesShapes,
         left = FALSE)
```
...
TravelTimeToWorkTractJoinedFilePath <- here("output/cache-data/08-expanding-transportation-socioeconomic/TravelTimeToWorkTractJoined.qs")


if(file.exists(ValidTablesLastYearFilePath)) {
  TravelTimeToWorkTractJoined <- qs2::qs_read(TravelTimeToWorkTractJoinedFilePath)
}
```
...
TravelTimeToWorkTractJoinedList$overlaps <-
  apply_intersection_by_id(TravelTimeToWorkTractJoinedList$overlaps,
                           ZonesShapes,
                           id_col = "OBJECTID")

TravelTimeToWorkTractJoined <- do.call("rbind", TravelTimeToWorkTractJoinedList)
```
...
qs2::qs_save(TravelTimeToWorkTractJoined, TravelTimeToWorkTractJoinedFilePath)
```
...
tm_shape(TravelTimeToWorkTractJoined) +
  tm_polygons(fill = "estimate")
```
...
```{r}
TravelTimeToWorkTractJoined$tract_area <- st_area(TravelTimeToWorkTractJoined$geometry)

setDT(TravelTimeToWorkTractJoined)

TravelTimeToWorkEstimateByZoneId <-
  TravelTimeToWorkTractJoined[
    j = .(estimate  = sum(estimate * 
                            as.double(tract_area /
                                        sum(tract_area, na.rm = TRUE)),
                          na.rm = TRUE)),
    by = c("LocationID", "variable")
  ][, dcast(.SD, LocationID ~ variable, value.var = "estimate")]
