...
gt(TopWordsByCluster) |>
  ...
  data_color(
    ...
    fn = scales::col_numeric(
      palette = c("white", ColorGray, ColorHighlight),
...
| Cluster ID | Top Words                          | Node Count | Reason for Inclusion                                                                 | Key Metrics to Extract                                                                 |
|------------|------------------------------------|------------|--------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|
| **18**     | Work, Transportation, Means, Time, Workplace | 32         | **Direct commuting behavior** - primary driver of taxi demand patterns               | - Commute mode share (% taxi/public transit)<br>- Average commute time<br>- Work-from-home rate |
| **13**     | Population, Years, Civilian, Employed, Age | 32         | **Employment density** - correlates with business/event-related trips              | - Employment-to-population ratio<br>- % working-age population (25-54)<br>- Job density |
| **36**     | Households, Income, Earnings, Dollars, Security | 19         | **Income capacity** - strongest predictor of taxi affordability and usage frequency | - Median household income<br>- Income density ($/mi²)<br>- % high-income households (>$100K) |


### Next Steps:
1. Extract variables from selected clusters
2. Calculate zone-level metrics:
   - **Commuter ratios** (cluster 18)
   - **Employment-to-population ratios** (cluster 13)
   - **Income density** (clusters 36)
   - **Poverty concentration** (cluster 20)
   - **Housing cost burden** (cluster 21)
3. Validate correlation with `take_current_trip`

As some variable show up in more than one dataset let's define a priority selection

Ok entonces para mis fines como estoy analizando NYC buscando patrones estables entre distintos años dare preferencia a los siguientes datasets
...
ConceptConsolidation <-
  unique(ValidTablesLastYear[, .(concept,
                                 year,
                                 dataset,
                                 survey)])


ConceptsToExploreClustersFinalVariables <-
...
ConceptsToExploreClustersFinalVariables[, .(N = .N, dataset = paste(dataset, collapse = ",")), node][N > 1, .N, dataset]
```
