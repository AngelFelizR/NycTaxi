...
## Transforming data based on base recipe
...
pin_write(
  BoardLocal,
  TrainingSampleAllFeatures,
  name = "TrainingSampleAllFeatures",
  type = "qs2",
  title = "Training Sample All Features"
)
```
...
identical(TrainingSampleJoined, TrainingSampleAllFeatures)

TrainingSampleAllFeatures <- pin_read(
  BoardLocal,
  name = "TrainingSampleAllFeatures"
)
```
...
WorkflowsToImport <-
  pin_list(BoardLocal) |>
  grep(pattern = "Wf$", value = TRUE)

names(WorkflowsToImport) <-
  WorkflowsToImport |>
  gsub(pattern = "Initial|Wf", replacement = "")
...
NycTaxi::compare_model_predictions(
  predictions_df = ExplainTestingPredictions,
  model_col = "wflow_id",
  model1_name = "Logistic",
  model2_name = "Xgboost",
  pivot_wider_id_cols = "trip_id"
) |>
  gt() |>
