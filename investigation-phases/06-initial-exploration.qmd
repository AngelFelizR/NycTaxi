...
plot_num_distribution <- function(dt,
  ...
  dt[, .value_prop := get_prop(.value), env = list(.value = x_str)]

  ecdf_plot =
    ggplot2::ggplot(dt, ggplot2::aes({{x}}, .value_prop)) 
  

  if(fill_str != "NULL") {

    dt[, j = {
      ecdf_fun = stats::ecdf(.value)
      new_dt = copy(.SD)[, .value_prop := ecdf_fun(.value)]
      new_dt
    },
    by = fill_str,
    env = list(.value = x_str)
    ]
    
    ecdf_plot =
      ggplot2::ggplot(dt, ggplot2::aes({{x}}, .value_prop, color = {{fill}}))
  }
  ...
  if(fill_str == "NULL" && !is.null(curve_important_points)){
    
    important_df = data.table(x = curve_important_points,
                              .value_prop = get_prop(curve_important_points))
    ...
    ecdf_plot =
      ...
      ggplot2::geom_text(data = important_df,
                         ggplot2::aes(label = scales::percent(.value_prop, accuracy = 1)),
...
We also know that many of the levels of the variables are present in less than 1% of the trips, so we are going to exclude those cases from the chart to avoid showing patterns that won't improve the model performance. If after making that exclusion there's only one level available, we are also excluding that variable from the chart to only play attention to important patterns.

After applying those filter we can say that:

1. `dispatching_base_num`
2. `originating_base_num`
3. `PULocationID`
4. `DOLocationID`
5. `access_a_ride_flag`
6. `wav_match_flag`
7. `company`
8. `weekday`
9. `month`
...
TrainingSample[trip_time_min >= 2,
  ...
  ggplot(aes(value, fill = factor(take_current_trip, levels = c("Y", "N")))) +
  geom_bar(width = 0.7,
           position = "fill") +
