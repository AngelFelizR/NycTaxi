...
4. **Extract new features**:  
  ...
  - For each zone and tag:  
     ...
     - Number of OSM points per $ft^2$.  
     - Total length ($ft$) of OSM lines per $ft^2$.  
     - Total area ($ft^2$) of OSM polygons per $ft^2$.  

5. **Integration**:  

   - Merge features with training data on zone ID.  

6. **Validation**:  

   - Plot feature distributions vs. `take_current_trip`.
   - Correlation of new predictos and target variable
...
After making those changes can consolidate all the data to use into 3 **geometry types**:
...
CountPlacesRegion <-
  ...
  ][, dcast(.SD, LocationID ~ variable, value.var = "N")]
```
...
AllPointsZones[, .(N = .N), 
                  by = .(value = paste0(toupper(feature)," ", value))
][order(-N)
][1:50
][, value := reorder(value, N, FUN = sum)] |>
  ggplot()+
  geom_col(aes(N, value,
               fill = N),
           position = position_dodge(),
           show.legend = FALSE)+
  scale_x_continuous(transform = "log2",
                     breaks = c(0, 2^seq(0, 24, by = 2)),
                     label = scales::label_comma(accuracy = 1))+
  scale_fill_gradient(high = ColorHighlight,
                      low = ColorGray,
                      breaks = c(0, 2^seq(0, 24, by = 2)),
                      transform = "log2",
...
AllLinesZones[, .(N = .N,
                  total_length = sum(st_length(geometry)) |> units::set_units(m)), 
              by = .(value = paste0(toupper(feature),": ", value))
][, value := reorder(value, N, FUN = sum)] |>
  ggplot()+
  geom_col(aes(total_length, value,
               fill = N),
           position = position_dodge())+
  scale_x_units(transform = "log2",
                breaks = c(0, 2^seq(0, 24, by = 2)),
...
AllPolygonZones[, .(N = .N,
                    total_area = sum(st_area(geometry)) |> set_units(m^2)), 
              by = .(value = paste0(toupper(feature),": ", value))
][order(-total_area)
][1:50
][,value := reorder(value, N, FUN = sum)] |>
  ggplot()+
  geom_col(aes(total_area, value,
               fill = N),
           position = position_dodge())+
  scale_x_units(transform = "log2",
                breaks = c(0, 2^seq(0, 24, by = 2)),
