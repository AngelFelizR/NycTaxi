...
MultiLinesInSeveralZonesCropped <- apply_intersection_by_id(MultiLinesInSeveralZones, ZonesShapes)
PolygonInSeveralZonesCropped <- apply_intersection_by_id(PolygonInSeveralZones, ZonesShapes)
MultiPolygonInSeveralZonesCropped <- apply_intersection_by_id(MultiPolygonInSeveralZones, ZonesShapes)
```
...
LinesWithinZonesDT  <- as.data.table(LinesWithinZones)

LinesWithinZonesDT[, .(N = .N,
                       line_length = median(st_length(geometry)) |> units::set_units(m)), 
                   .(feature, 
                     value)
][,value := reorder(value, line_length, FUN = sum)] |>
ggplot()+
  geom_col(aes(line_length, value,
               fill = N),
...
## Consolidating information to week
...
```{r}
PointsWithinZonesDT <- as.data.table(PointsWithinZones)


CountPlacesRegion <-
  PointsWithinZonesDT[, .N, 
                      .(LocationID = OBJECTID, 
                        feature, 
                        value)
  ][, dcast(.SD, 
            formula = LocationID ~ feature + value, 
            value.var = "N",
            fill = 0L)]


CountCorrelation <-
  CountPlacesRegion[TrainingSample[, .(trip_id,
...
CountCorrelation[, "take_current_trip"] |> abs() |> sort(decreasing = TRUE) |> head()

CountCorrelation[, "performance_per_hour"] |> abs() |> sort(decreasing = TRUE) |> head()
```
