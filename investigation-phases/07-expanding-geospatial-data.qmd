...
Now we can safe the transformed data for future use.

```r
fst::write_fst(TrainingSampleOmsDensityFeatures, 
               path = here("output/TrainingSampleOmsDensityFeatures.fst"))
```
...
As we can see, the variables **don't show a strong correlation** (<10% for the top cases) with the variable to predict, so we will need to go deeper on feature engineering before using these variables for modeling.
