...
bb_box <-
  lapply(X = paste0(BoroughSelected, ", NY"), FUN = osmdata::getbb) |>
  do.call(what = "cbind") |>
...
if (!exists("bb_box")) {
  bb_box <- pin_read(BoardRemote, "bb_box")
}
```
...
if (!exists("OsmDataComplete")) {
  ...
  OsmDataComplete <- pin_read(BoardRemote, "OsmDataComplete")
}
...
if (!exists("PointsWithinZones")) {
  PointsWithinZones <- pin_read(BoardRemote, "PointsWithinZones")
}

if (!exists("LinesWithinZones")) {
  LinesWithinZones <- pin_read(BoardRemote, "LinesWithinZones")
}

if (!exists("MultiLinesWithinZones")) {
  MultiLinesWithinZones <- pin_read(BoardRemote, "MultiLinesWithinZones")
}

if (!exists("PolygonWithinZones")) {
  PolygonWithinZones <- pin_read(BoardRemote, "PolygonWithinZones")
}

if (!exists("MultiPolygonWithinZones")) {
  MultiPolygonWithinZones <- pin_read(BoardRemote, "MultiPolygonWithinZones")
}
```
...
if (!exists("LinesInSeveralZones")) {
  LinesInSeveralZones <- pin_read(BoardRemote, "LinesInSeveralZones")
}

if (!exists("MultiLinesInSeveralZones")) {
  MultiLinesInSeveralZones <- pin_read(BoardRemote, "MultiLinesInSeveralZones")
}

if (!exists("PolygonInSeveralZones")) {
  PolygonInSeveralZones <- pin_read(BoardRemote, "PolygonInSeveralZones")
}
...
rm(
  list = grep(
    "OmsDensityFeatures|TrainingSample|Color|Board",
    ls(),
    invert = TRUE,
    value = TRUE
  )
...
ggplot(TopOsmCorrelations) +
  ...
  scale_fill_manual(values = c("TRUE" = ColorHighlight, "FALSE" = ColorGray)) +
  scale_x_continuous(
