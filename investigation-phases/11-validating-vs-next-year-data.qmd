...
As we use using data from 2022-2023 to train and tune our policy in this chapter, we are going to aproximate how the created policy would work on production by been used by the taxi drivers.

## Defining Enviroment

We first need to load all the functions to be used.
...
Know we just repeat the steps we applyied to download all the data and consolidating evething into a the new table `NycTrips2024` in our duckdb database.
...
## Saving a sample from 2024

We have all the data we just need to select a sample to be taken as starting point for simulating each day.
...
Now we are ready to simulate the **1,058 days** of 2024 with selecting all trips vs implementing 2 policies created.
...
Now we can say without any fear the the recipe found in general and not the result of overfitting.
