...
Execute the command below to run the container based on the `angelfelizr/nyc-taxi-nix:4.5.1` image. This command will:
  ...
  * Mount the cloned repository to the `/home/nyc/NycTaxi` directory within the container (`-v <parent-dir-path>/NycTaxi:/home/nyc/NycTaxi`).
...
docker run --rm -tid \
  ...
  -p 2222:22 \
  -v ~/r-projects/NycTaxi:/home/nyc/NycTaxi \
  -v ~/r-projects/NycTaxiPins:/home/nyc/NycTaxiPins \
  -v ~/r-projects/NycTaxiBigFiles:/home/nyc/NycTaxiBigFiles \
  angelfelizr/nyc-taxi-nix:4.5.1 && sleep 2 && \
  docker cp ~/.ssh/id_rsa.pub nyc-taxi-container:/home/nyc/.ssh/authorized_keys && \
  docker exec nyc-taxi-container chown nyc:nyc /home/nyc/.ssh/authorized_keys && \
  docker exec nyc-taxi-container chmod 600 /home/nyc/.ssh/authorized_keys
```
...
```bash
ssh nyc@localhost -p 2222
```

2.  **Using Positron**: Select the **"Connect to Host"** option in **Positron** and enter `nyc@localhost:2222` with `"r"` as the password. Then we just need to install the [direnv](https://open-vsx.org/vscode/item?itemName=mkhl.direnv) extention by **mkhl**.
