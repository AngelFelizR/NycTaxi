...
Now the strategy will be to undertand what patterns have been catched by `xgboost` that we not  catched by the `Logict Regression` and then define whether we need to create modefy the features of `Logict Regression` or create an statck model to use the best of both models.
