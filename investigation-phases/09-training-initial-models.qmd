...
ModelComparison |>
  ...
  data_color(
    ...
    fn = scales::col_numeric(
      palette = c("#d4edda", "white"), # Green for lower (better) values
      domain = c(min(ModelComparison$mean), max(ModelComparison$mean))
    )
  ) |>
  ...
  # 7. Source Note
  tab_source_note(
    source_note = "Metrics extracted from BoardLocal pins: MetricsXgboostHistory and WorkFlowTreesTuned"
  )
```
...
The results doesn't match with what were expecting. As the top
...
bind_rows(
  ...
  labs(
    title = "Top variables were reported as negative",
    x = "",
    y = "Brier Class Change"
  ) +
  theme_minimal() +
  theme(
    panel.grid.major.y = element_blank(),
...
ggplot(ExplainerXgboostConsolidation, aes(x = Gain, y = dropout_loss)) +
  ...
  geom_hline(
    ...
    color = "gray",
    linewidth = 0.5
  ) + # Línea en 0 para resaltar negativos
  geom_text(
    aes(label = variable),
    size = 3,
...
```{r}
BestXgboostConfigPls <- pin_read(
  BoardLocal,
  name = "BestXgboostConfigPls"
)

MetricsXgboostHistoryPls <- pin_read(
  BoardLocal,
  name = "MetricsXgboostHistoryPls"
)

MetricsXgboostHistoryPls |>
  filter(.metric == "brier_class") |>
