...
```{r}
WorkFlowTunedBest |>
  mutate(model = reorder(model, -mean)) |>
...
```{r}
WorkFlowTunedBest |>
  filter(grepl("reduced_levels|normalized", model)) |>
  arrange(mean, std_err) |>
  group_by(model) |>
  slice_head(n = 1) |>
  transmute(model = paste(model, "-", .config), .metric, mean, std_err) |>
  arrange(mean, std_err) |>
  ungroup() |>
  mutate(model = factor(model, levels = rev(model))) |>
  ggplot(aes(x = mean, y = model)) +
  geom_errorbar(aes(xmin = mean - 3 * std_err, xmax = mean + 3 * std_err)) +
  geom_point() +
  labs(x = "brier_class", y = "Best Models") +
  theme_light()
```
