...
After explorting the data and getting new features it's time to explore the data using ML models to extract insights that will be used to create the final model.

In this section we will start defining the models to train. We prefer using memory efficient models as we are working with hardware limitations. In this case willl train:
...
We are going to fit 10 radom variations of the different parameters to tune over 5 fold cross validation, then explore the results of each model.
...
AcsVariablesByZoneId <-
  pin_read(BoardRemote, "AcsVariablesByZoneId")[,
...
TrainingSampleJoined <- pin_read(BoardRemote, "TrainingSampleJoined")
```
...
qs2::qs_save(WorkFlowToTune, here("../NycTaxiBigFiles/WorkFlowToTune.qs"))
```
