...
PredictionsCosts |>
  ...
  labs(
    title = paste0(
      ...
      "**Logistic Regression**",
      "</span> ",
      "losses<br>less money than current method"
    ),
    y = "Trained Models",
...
After evaluating multiple modeling approaches, **regularized logistic regression emerged as the optimal production model** for this trip acceptance problem, achieving:

- **Best cost performance**: Reduced average loss per trip from $3.8 (baseline accept-all strategy) to approximately $2.6
- **Strong predictive accuracy**: Competitive Brier Score compared to more complex models
- **Production advantages**: Interpretable, lightweight, and fast enough for real-time driver recommendations

### Key Insights

While **random forest** showed comparable raw prediction accuracy, it **failed to translate this into better cost performance**. This reveals an important lesson: the cases where random forest outperforms logistic regression are not the economically significant ones. The logistic regression correctly identifies the high-cost mistakes that matter most to driver profitability.

Interestingly, **pls_mars** achieved similar cost reduction despite a higher Brier Score, suggesting that model comparison should always be tied to business metrics rather than purely statistical ones.

### Model Improvement Opportunities

The complementary strengths of logistic regression and random forest suggest potential for further optimization if we:

1. **Identify disagreement cases**: Analyze observations that **reduced_levels_random_forest** predicts correctly but **normalized_reg_logistic** fails to predict

2. **Apply instance-level explainability**: Use techniques like SHAP or Break-down Plots to understand the reasons behind each model's predictions

3. **Engineer targeted features**: Modify **normalized_reg_logistic** features or create a stacked ensemble to capture the best aspects of both models

These improvements would be explored in the next phase of model development, building on the strong foundation of the current production model.

## Saving best workflow

After knowing the next steps we just need to save the best workflows for **normalized_reg_logistic** and **reduced_levels_random_forest** to be used in the next section.
...
InitialRandomForestWf <- WorkFlowTrees |>
  extract_workflow(id = "reduced_levels_random_forest") |>
  finalize_workflow(
    parameters = select_best(
...
pin_write(
  ...
  "InitialRandomForestWf",
  type = "qs2",
  title = "Initial Random Forest Wf"
)
```