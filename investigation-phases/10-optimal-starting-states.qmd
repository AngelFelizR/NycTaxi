...
Once have a policy to decide if a trip should be taken or rejected then it's important to validate if any of the next factors could cause an increase of decrease on taxi results:
...
::: {.callout-note}
As the `DOLocationID`is the variable used in the simulation to define the start zone of the fist trip to simulate we need to simulate all avaiable zones for each combination.
:::
...
Now we just need to take that information in format that can be applyied to the strips sampled from the data.
...
All this logic information was implemented in the `optimize_trip_start_time` and we can make sure the the trips fills this conditions after applying the functions over the trips used to create the baseline.
...
To evaluate this policy under the same conditions we evaluete the baseline and the accept/reject policy we are using the same simulation with the difference that now we are passing the `start_day_fitted_model` and `valid_start_times_dt` arguments that will be use by the `optimize_trip_start_time` to change initial condition of each day.
...
Now we can consolidate all results in under to comparete the 3 strategies under the same conditions.
```{r}
...
The results are amizing we move from having **\$`r round(FinalPfaMeans[1L], 2)` per hour** to be able to earn on average **\$`r round(FinalPfaMeans[3L], 2)`** if we implement both policies at the same time. We also con see that the chart present outliers of days that ended earning more than **\$90** that was not possible even using the accept/reject policy.
...
ggplot(FinalPfaResultsSummary, aes(strategy, daily_hourly_wage_mean)) +
  ...
  geom_hline(yintercept = FinalPfaMeans[3L], linetype = 2) +
  scale_fill_manual(
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      dollar(FinalPfaMeans[3L], accuracy = 1),
...
If we compare the difference between the accept/reject policy and the policy created now to start the trips  we can confirm that the increase on the average is no due an increase on variability but because that in the mayority of cases the taxis drivers are getting better results that the original situation.
