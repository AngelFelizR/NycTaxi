...
As we know that this rule only works for **Uber**, then the first before validating with the tree model if the time is good is to make sure that the company for each start trip correspond to Uber.

Then we will use the tree model see if the time is correct to start working of we should wait until finding the **next valid hour** to start working.

Now we just need extract the valid hours and used as part of t reference for the simulation.
...
```{r}
setDT(DecisionTreeAugmented)

update_start_trip <- function(
  trips_dt,
  ...
  stopifnot(
    "valid_times_dt must be a data.table" = data.table::is.data.table(
      valid_times_dt
    )
  )

  # Validating the the dataframes present the expected columns
  if (!all(c("hvfhs_license_num", "request_datetime") %chin% names(trips_dt))) {
    stop(
      "trips_dt must have `hvfhs_license_num` and `request_datetime` as columns."
    )
  }
  if (!all(c("hour", "week_day", "week_cycle") %chin% names(valid_times_dt))) {
    stop(
      "valid_times_dt must have `hour`, `week_day`, and `week_cycle` as columns."
    )
  }

  # Adding the possibility that the next valid hour will take place next week
  valid_times_next_week =
    data.table::copy(valid_times_dt)[
      j = week_cycle := week_cycle + 7 * 24
    ][j = rbind(valid_times_dt, .SD)]
}


DecisionTreeAugmentedAsNo <- DecisionTreeAugmented[.pred_class == "no"]
DecisionTreeAugmentedAsNo[, hvfhs_license_num := "HV0003"]

DecisionTreeAugmentedAsNo[
  j = request_datetime := {
    ...
    next_week_cycle = ValidHoursToStartWorking[
    ...
    new_request_datetime = request_datetime +
