...
2. Available **after arriving** ending the trip, so can be used to estimate the **outcome** to predict.
...
::: {.callout-note}
Despite **trip_time** and **driver_pay** are usually predicted by the company's apps are are assuming that those fields represent the final value after completing each trip and not an estimation.
:::
