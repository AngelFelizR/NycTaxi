...
### Updating the probability threshold for production

Now that the simulation has identified the **optimal operational threshold** ($\tau^* = `r round(BestResult$tau_used, 2)`$), we can embed it directly into the final fitted workflow. This creates a production-ready model object that applies the exact decision rule we validated in the full-day Monte-Carlo simulator.

```{r}
#| label: Adding threshold to model
#| eval: false
