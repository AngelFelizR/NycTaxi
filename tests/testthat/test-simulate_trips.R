describe("simulate_trips()", {
  it("only accepts connections with NycTrips and PointMeanDistance", {
    ...
    SimulationStartTrips = readRDS(test_path(
      "fixtures",
      "SimulationStartTrips.rds"
    ))
  ...
  it("NycTrips must have the correct columns", {
    ...
    DBI::dbWriteTable(
      temp_con,
      "PointMeanDistance",
      DBI::dbReadTable(test_con, "PointMeanDistance")
    )
  ...
  it("PointMeanDistance must have the correct columns", {
    ...
    DBI::dbWriteTable(
      temp_con,
      "NycTrips",
      DBI::dbReadTable(test_con, "NycTrips")
    )
  ...
  it("simulates as expected", {
    ...
    expect_equal(
      simulation_example[simulation_id == sim_trip_id, `sim_trip_id`],
    ...
    expect_equal(
      simulation_example[,
        .(unique_count = uniqueN(sim_hvfhs_license_num)),
        by = "simulation_id"
      ][, unique_count],
      c(1L, 1L)
    )
    ...
    expect_equal(
      simulation_example[,
        .(unique_count = uniqueN(sim_wav_match_flag)),
        by = "simulation_id"
      ][, unique_count],
      c(2L, 1L)
    )
    ...
    last_trip_hours_working =
      simulation_example[,
        .(
          min_time = min(sim_request_datetime),
          max_time = max(sim_request_datetime)
        ),
        by = "simulation_id"
      ][,
        diff_hours := difftime(max_time, min_time, units = "hours") |>
          as.double()
      ]
    ...
    expect_equal(
      simulation_example$sim_trip_id,
      # UBER TRIPS
      c(
        218799831,
