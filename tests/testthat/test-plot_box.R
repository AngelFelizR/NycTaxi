...
test_that("plot_box handles invalid inputs appropriately", {
  ...
  expect_error(plot_box(data.frame(x = 1, y = 2)), "x must be a matrix")

  # Test 5: Matrix with incorrect column names
  bad_colnames <- matrix(1:4, ncol = 2)
  expect_error(
    plot_box(bad_colnames),
  ...
  expect_error(plot_box(char_matrix), "Matrix must contain numeric values")

  # Test 7: Matrix with wrong number of rows
  three_row <- matrix(1:6, ncol = 2, dimnames = list(NULL, c("x", "y")))
  expect_error(plot_box(three_row), "Input matrix must have exactly 2 rows")
})
