validate_simulation_data = function(conn, start_points) {
  ...
  check_db_columns(conn, "NycTrips", min_trip_info)
  check_db_columns(
    conn,
    "PointMeanDistance",
    c('PULocationID', 'DOLocationID', 'trip_miles_mean')
  )

  # Validating start trips
  stopifnot(
    "start_points must be a data.table" = data.table::is.data.table(
      start_points
    )
  )
...
check_db_columns = function(conn, table_name, col_names) {
  db_table_cols =
    DBI::dbGetQuery(
      conn = conn,
