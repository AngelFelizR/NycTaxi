...
#'
#' @export
add_take_current_trip <- function(
  trip_sample,
  point_mean_distance,
  parquet_path,
  future.scheduling = 1,
  ...
  if (length(missing_cols) > 0) {
    stop(paste(
      "Missing required columns in trip_sample:",
  ...
  tryCatch(
    {
      ...
      all_trips <- DBI::dbGetQuery(
        con,
        paste0("SELECT * FROM '", parquet_path, "'")
      )
  ...
  select_valid_future_trips <- function(n_row) {
    ...
    distance_to_use <- point_mean_distance[
      ...
      on = "PULocationID",
      nomatch = NULL,
      j = .(DOLocationID = PULocationID, trip_miles_mean)
    ] |>
    ...
    valid_future_trips <- all_trips[
      ...
      request_datetime >=
        (trip_sample_i$request_datetime + lubridate::seconds(3)) &
        request_datetime <=
          (trip_sample_i$request_datetime + lubridate::minutes(15)) &
      ...
      (request_datetime <=
        (trip_sample_i$request_datetime + lubridate::minutes(1)) &
        trip_miles_mean <= 1) |
        (request_datetime <=
          (trip_sample_i$request_datetime + lubridate::minutes(3)) &
          trip_miles_mean <= 3) |
        (request_datetime <=
          (trip_sample_i$request_datetime + lubridate::minutes(5)) &
          trip_miles_mean <= 5) |
        (request_datetime <=
          (trip_sample_i$request_datetime + lubridate::minutes(7)) &
          trip_miles_mean <= 7) |
        (request_datetime <=
          (trip_sample_i$request_datetime + lubridate::minutes(9)) &
          trip_miles_mean <= 9) |
        (request_datetime <=
          (trip_sample_i$request_datetime + lubridate::minutes(11)) &
          trip_miles_mean <= 11) |
        (request_datetime <=
          (trip_sample_i$request_datetime + lubridate::minutes(13)) &
          trip_miles_mean <= 13) |
        (request_datetime <=
          (trip_sample_i$request_datetime + lubridate::minutes(15)) &
          trip_miles_mean <= 15)
    ...
    valid_future_trips[,
      waiting_secs := as.numeric(difftime(
        request_datetime,
        trip_sample_i$request_datetime,
        units = "secs"
      ))
    ]
    valid_future_trips[,
      performance_per_hour := (driver_pay + tips) /
        ((trip_time + waiting_secs) / 3600)
    ]
  ...
  future_trip_summary <- trip_data[
    j = .(
      percentile_75_performance = stats::quantile(
        performance_per_hour,
        0.75,
        na.rm = TRUE
      )
  ...
  final_data <- future_trip_summary[trip_sample, on = "trip_id"][,
    take_current_trip := data.table::fifelse(
  ...
  final_data[is.na(take_current_trip), take_current_trip := 1L]

  # Define order for new columns
  data.table::setcolorder(
    final_data,
    "percentile_75_performance",
    before = "take_current_trip"
  )
