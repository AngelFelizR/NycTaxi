...
I'm fine with horizontal dimension, so we only need to change the vertical coordinates.

```{r}
bb_box_manual <- bb_box
bb_box_manual["y", ] <- c(40.541978, 40.9166) 

project.nyc.taxi::plot_box(bb_box_manual) +
  tmap::tm_shape(ZoneCodesRefSfClean) +
  tmap::tm_symbols(size = 0.5,
                   fill.scale = tm_scale_categorical(values = BoroughColors),
                   fill = "Borough",
...
**Focus Areas & Feature Types**


Here's a breakdown of relevant OSM features:

1.  **Point of Interest (POI) Density/Type**:
    *   **Why it matters:** Your EDA shows that some locations have consistently high performance. This could be driven by the kind of places they contain. Areas with dense restaurants or hotels, for example, will have more riders.
        *   **Features to extract:**
            *   Count of different POI categories (restaurants, bars, hotels, tourist attractions, shops, ) within a radius of each pickup and drop off location
            *   Density of specific POIs within a radius (e.g. restaurants per sq km). You might want to separate the "high-end" from the "low-end" (fast food) locations.
    

2.  **Road Network Characteristics:**
    *   **Why it matters:** The density and type of roads can impact traffic, which can influence trip times and therefore earnings per hour.
        *   **Features to extract:**
            *   Total road length within a given radius of the pick up and drop off locations, also consider the type of road (`highway=*`)
            *   Density of different types of roads (e.g. highways, main roads, residential streets).
            *   Count of intersections within a defined radius
            *   Average speed limit of roads in the area.
    *   **OSM Tags:** `highway=*`, `maxspeed=*`

3.  **Transit Information:**
    *   **Why it matters:** Areas near transit hubs are likely to have more demand and impact how people use taxis
        *   **Features to extract:**
