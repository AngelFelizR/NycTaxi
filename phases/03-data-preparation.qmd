...
The process was implemented in the **`project.nyc.taxi::add_take_current_trip()`** which has been **tested** to ensure it meets expectations, as confirmed in the [Github file](https://github.com/AngelFelizR/project.nyc.taxi/blob/master/tests/testthat/test-add_take_current_trip.R).
...
#| echo: true
#| eval: false
#| file: ../R/01-fine-tune-future-process.R
```
...
1.  Importing the data `PointMeanDistance` as a *data.table*.

```{r}
PointMeanDistance <- read_fst(here("output/cache-data/PointMeanDistance.fst"), as.data.table = TRUE)
```
