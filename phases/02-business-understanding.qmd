...
They will be used as the __predictors__ to train the model.
...
Based on the simulation's results we can confirm that the average earnings for a taxi driver per hour goes between 44.26 and 50.59, but that doesn't represent the highest values observed on the simulation.
...
If we can apply a strategy that can move the `Daily Hourly Wage` to 60 dollars per hour ( $ \frac{60 - 44.26}{44.26} = 36\%$ ) if would be a be a great change for many taxi drivers if they apply this strategy.
...
But to address the second question, we need to create the variable `take_current_trip` indicating whether the current trip should be taken based on the future trips that can be taken in the future from that particular position.
