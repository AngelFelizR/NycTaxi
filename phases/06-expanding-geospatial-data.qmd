...
Ahora estamos pendiente de crear una funcion que pueda crear plots con todos los objetos osm para revisar la informacion disponible.

```{r}
sapply(LandUseData, \(x) !is.null(x) && inherits(x, "sf")) 

tm_shape(LandUseData$osm_points) +
  tm_symbols(fill = "landuse",
             fill.scale = tm_scale_categorical(n.max = 50)) +
  tm_shape(LandUseData$osm_polygons) +
  tm_polygons(fill = "landuse",
              fill.scale = tm_scale_categorical(n.max = 50)) +
  tm_shape(LandUseData$osm_multipolygons) +
  tm_polygons(fill = "landuse",
              fill.scale = tm_scale_categorical(n.max = 50)) +
  tm_basemap("CartoDB.Positron")
```
