...
-   Uber accounts for 72% of trips. In consequence, the results of the this project will be **more related to Uber than Lyft**.

-   `dispatching_base_num`, `originating_base_num` and `access_a_ride_flag` all seem to be highly correlated with company and are **unlikely to be useful for prediction**.
...
To explore the distribution provided by the `PULocationID` (Pick Up Zone ID) and `DOLocationID` (Drop Off Zone ID) we need need to add extra information. If we check the taxi zone look up table we can find the `Borough` and `service_zone` of each table.
...
1.  The distributions of pick ups and drop offs positions is the same, across boroughs, service zones and individual zones.

2.  Must of the zones have have 2000 or less in the data and only few have more than 3000 trips in the training set.

3.  Must of the trips take place in Manhattan and in the Boro Zone.
...
(PuBoroughPlot / DoBoroughPlot) +
  plot_annotation(title = 'Most the trips start and end in Manhattan') & 
...
(PuServiceZonePlot / DoServiceZonePlot) +
  plot_annotation(title = 'Most of the trip take place in the Boro Zone') & 
  theme(plot.title = element_text(face = "bold", size = 15))
```
:::
...
TripPerformanceRangePlot <-
  ...
  scale_y_continuous(labels = percent_format(),
                     n.breaks = 5) +
  labs(subtitle = "Tips with 5 min and positive base fare",
       x = "") +
  theme_minimal()
...
As we can see below, the proportion of trips to take change for the next features:
...
TrainingSampleZones[trip_time >= 60*5 & 
                      base_passenger_fare > 0 &
                      performance_per_hour > 0, 
                    .SD, 
                    .SDcols = patterns("^(PU|DO)|take_current_trip|license|shared|wav|request_datetime")
         ...
         weekday = wday(request_datetime, label = TRUE, abbr = FALSE, week_start = 1),
         month = month(request_datetime, label = TRUE, abbr = FALSE),
         hour = hour(request_datetime) |> factor())] |>
  plot_bar(by = "take_current_trip",
           nrow = 2,
...
TrainingSampleZones[trip_time >= 60*5 & 
                      base_passenger_fare > 0 &
                      performance_per_hour > 0] |>
  ggplot(aes(take_current_trip, trip_miles)) +
  geom_boxplot() +
...
As we can see in the below charts, __categorical predictors don't seem to provide a very impacting variable__ for modeling at a first glance, as the distribution of performance_per_hour is very similar in multiple categories.
...
- On average, the `performance_per_hour increases` with miles, specially for trips with more than 30 miles.
...
TrainingSampleZones[trip_time >= 60*5 & 
                      ...
                      performance_per_hour > 0, 
                    .SD, 
                    .SDcols = patterns("^(PU|DO)|performance_per_hour|license|shared|wav|request_datetime")
][, `:=`(PULocationID = forcats::fct_lump_n(as.character(PULocationID), 19),
         DOLocationID = forcats::fct_lump_n(as.character(DOLocationID), 19),
         request_datetime = NULL,
         weekday = wday(request_datetime, week_start = 1) |> factor(),
         month = month(request_datetime) |> factor(),
         hour = hour(request_datetime) |> stringr::str_pad(2, pad = "0") |> factor())] |>
  melt(id.vars = "performance_per_hour") |>
...
TrainingSampleZones[trip_time >= 60*5 &
                      base_passenger_fare > 0 &
                      performance_per_hour > 0] |>
  ggplot(aes(trip_miles, performance_per_hour))+
...
After focusing on identifying high-value trips, most of the **categorical predictors reveal more information that could be useful for making predictions**. The distributions of high-value and not-high-value trips show noticeable differences across several categories, although these differences are much more pronounced on some categories than on others. The `wav_match_flag` stands out as an exception with virtually identical distributions for both groups.

The `trip_miles` boxplot confirms that high-value trips are typically associated with short distances. 
...
TrainingSampleZones[trip_time >= 60*5 & 
         ...
         high_value_trip = fifelse(performance_per_hour >= 100, "Y", "N"),
         PULocationID = forcats::fct_lump_n(as.character(PULocationID), 29),
         DOLocationID = forcats::fct_lump_n(as.character(DOLocationID), 29),
         request_datetime = NULL,
         weekday = wday(request_datetime, label = TRUE, abbr = FALSE, week_start = 1),
...
TrainingSampleZones[trip_time >= 60*5 & 
                      base_passenger_fare > 0 &
                      performance_per_hour > 0] |>
  ggplot(aes(fifelse(performance_per_hour >= 100, "Y", "N"), trip_miles)) +
  geom_boxplot() +
  scale_y_continuous(n.breaks = 10) +
  theme_minimal() +
  theme(legend.position = "top",
...
En general, the only important correlations take place between variables related to trip location.
...
TrainingSampleZones[trip_time >= 60*5 & 
                      ...
                      performance_per_hour > 0, 
                    .SD, 
                    .SDcols = patterns("^(PU|DO)|license|shared|wav|request_datetime|trip_miles")
][, `:=`(PULocationID = forcats::fct_lump_n(as.character(PULocationID), 29),
         DOLocationID = forcats::fct_lump_n(as.character(DOLocationID), 29),
         request_datetime = NULL,
         weekday = wday(request_datetime, label = TRUE, abbr = FALSE, week_start = 1),
         month = month(request_datetime, label = TRUE, abbr = FALSE),
         hour = hour(request_datetime) |> factor(),
         trip_miles = cut_interval(trip_miles, 8))
  ][, corrcat::cramerV_df(.SD, unique = FALSE)] |>
