...
DaylyRequestPlot <-
  ggplot(TrainingSample,
         aes(as_date(request_datetime),
             text = paste0("Date: ", format(request_datetime, "%m-%d")))) +
  ...
  labs(title = "Distribution of dayly trip requets",
       x = "Day") +
  theme_minimal()+
...
ggplotly(DaylyRequestPlot, tooltip = c("text", "count"))
```
...
ggplot(TrainingSample, aes(request_datetime)) +
...
RequestTimeDetails[, .(n_trips = .N),
  ...
  geom_col(fill = "gray80",
           ...
           width = 0.8) +
  scale_x_continuous(labels = comma_format()) +
  labs(title = "Few samples for December",
       y = "",
       x = "Number of Trips") +
  theme_minimal() +
  theme(panel.grid.major.y = element_blank(),
