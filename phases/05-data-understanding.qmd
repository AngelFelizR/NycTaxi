...
```{r}
TrainingSampleZones[, .(take_current_trip,
                        ...
                        wav_request_flag,
                        wav_match_flag)] |>
  DataExplorer::plot_bar(by = "take_current_trip")


TrainingSampleZones[, .SD, 
                    .SDcols = patterns("^(PU|DO)|take_current_trip")
][, `:=`(PULocationID = forcats::fct_lump_n(as.character(PULocationID), 49),
         DOLocationID = forcats::fct_lump_n(as.character(DOLocationID), 49))] |>
  DataExplorer::plot_bar(by = "take_current_trip")
```
...
TrainingSampleZones[trip_time >= 60*5 & 
                      ...
                      performance_per_hour > 0, 
                    .SD, 
                    .SDcols = patterns("^(PU|DO)|performance_per_hour")
][, `:=`(PULocationID = forcats::fct_lump_n(as.character(PULocationID), 12),
         DOLocationID = forcats::fct_lump_n(as.character(DOLocationID), 12))] |>
    melt(id.vars = "performance_per_hour") |>
  ggplot(aes(value, performance_per_hour)) +
  geom_boxplot() +
  scale_y_log10() +
